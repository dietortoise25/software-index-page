      ...
      <Card>
        ...
        <CardContent>
          <ResponsiveContainer width="100%" height={240}>
            <PieChart><Pie data={data.pie} cx="50%" cy="50%" innerRadius={55} outerRadius={90} dataKey="value" label={({ name, value }) => { const total = data.pie.reduce((s, d) => s + d.value, 0); return `${name} ${total > 0 ? (value / total * 100).toFixed(0) : 0}%` }}>{data.pie.map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} />)}</Pie><Tooltip formatter={(v) => `BRL ${Number(v).toLocaleString("zh-CN", { minimumFractionDigits: 2 })}`} /></PieChart>
          </ResponsiveContainer>
        </CardContent>
      </Card>
      <Card>
