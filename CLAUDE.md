...
所有功能开发、Bug 修复、重构、行为变更都必须遵循 TDD 铁律：

**铁律**: 没有先看到测试失败，绝不写生产代码。
...
### 测试要求
- 一次只测一个行为，测试名清晰描述场景
- 用真实代码而非 mock（mock 仅在不可避免时使用）
- 在实现前先看到测试失败（证明测试确实在测东西）
- 永远不要先写代码再补测试

### 违禁行为
- 先写生产代码再补测试 → 删除代码，重新 TDD
- 测试直接通过 → 不是 TDD，测试没测到新功能
- Bug 修复杂而不写复现测试 → 禁止
- "这次太简单不需要测试" → 简单代码也会挂，30 秒写一个测试
