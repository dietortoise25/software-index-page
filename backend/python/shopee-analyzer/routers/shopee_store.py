"""
店铺分析 FastAPI 路由 —— 从 main.py 抽出的 HTTP 层（MVC 分层）。

职责仅限：入参校验 / 文件读取 / 调业务函数 / 拼返回。
店铺分析业务算法仍在 analyzer / extractors / metrics / diagnose / simulate /
aibuy_client 等模块，本路由仅做编排。

端点（与拆分前完全一致）:
  POST /api/analyze        上传单个 Shopee Excel，返回店铺分析
  POST /api/diagnose       上传店铺/广告/订单文件，返回分析 + ROI 诊断
  GET  /api/rules          读取诊断规则配置
  PUT  /api/rules          更新诊断规则配置（合并写入）
  POST /api/simulate       预算重分配模拟
  POST /api/search-image   以图搜货
  GET  /api/health         健康检查

安全：所有内部异常只写日志（logger.exception），返回客户端的是不含堆栈的
`{"code": "...", "message": "..."}` 结构。
"""
import logging
...
router = APIRouter(tags=["shopee-store"])

MAX_FILE_SIZE = 10 * 1024 * 1024


def _error(code: str, message: str, status: int = 500) -> HTTPException:
    """构造不含堆栈的错误响应（堆栈已在调用处写日志）。"""
...
async def analyze(file: UploadFile = File(...)):
    ...
    try:
        ...
        raise _error("analyze_failed", "店铺数据分析失败，请稍后重试或检查文件格式")
...
async def diagnose(
    ...
    if store_file:
        ...
        try:
            ...
            raise _error("store_analyze_failed", "店铺分析失败，请检查店铺统计文件")
    ...
    if ad_file:
        ...
        try:
            ...
            raise _error("ad_analyze_failed", "广告分析失败，请检查广告整体文件")
    ...
    if order_file:
        ...
        try:
            ...
            raise _error("order_analyze_failed", "订单分析失败，请检查订单明细文件")
...
async def get_rules():
    ...
    try:
        ...
        logger.exception("读取规则失败")
        raise _error("rules_load_failed", "读取诊断规则失败")
...
async def update_rules(body: dict):
    ...
    try:
        ...
        raise _error("rules_update_failed", "更新诊断规则失败")
...
async def simulate(body: dict):
    ...
    try:
        ...
        raise _error("simulate_failed", "预算模拟失败，请检查输入参数")
...
async def search_image(body: dict):
    ...
    try:
        ...
        raise _error("search_image_failed", "以图搜货失败，请稍后重试")
